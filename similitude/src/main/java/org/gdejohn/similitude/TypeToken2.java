...
public class TypeToken2<T> implements Type
{
	static final Logger LOGGER = getLogger(TypeToken2.class);
	
	private final TypeToken2<?> PARENT;
	
	private final Class<?> RAW_TYPE;
	
	private final TypeToken2<?> OWNER_TYPE;
	
	private final Map<TypeVariable<?>, TypeToken2<?>> TYPE_ARGUMENTS;
	...
	public TypeToken2(final Type TYPE, final TypeToken2<?> PARENT, final Set<Class<?>> IMMUTABLE, final boolean DETERMINE_IMMUTABLE, final Object... ARGUMENTS)
	{
		...
		if (TYPE instanceof GenericArrayType)
		{
			...
			final TypeToken2<Object> COMPONENT_TYPE =
			(
				new TypeToken2<Object>
				(
		...
		else
		{
			...
			{
				final TypeToken2<?> RESOLVED_TYPE =
				(
			...
			else if (TYPE instanceof ParameterizedType)
			{
				...
				TYPE_ARGUMENTS =
				(
					new LinkedHashMap<TypeVariable<?>, TypeToken2<?>>( )
				);
	...
	public TypeToken2(final Type TYPE, final TypeToken2<?> PARENT, final boolean DETERMINE_IMMUTABLE)
	{
		this(TYPE, PARENT, Cloner.BASIC_TYPES.keySet( ), DETERMINE_IMMUTABLE, new Object[0]);
	}
	
	public TypeToken2(final Type TYPE, final boolean DETERMINE_IMMUTABLE, final Object... ARGUMENTS)
	{
		this(TYPE, null, Cloner.BASIC_TYPES.keySet( ), DETERMINE_IMMUTABLE, ARGUMENTS);
	}
	
	public TypeToken2(final Type TYPE, final boolean DETERMINE_IMMUTABLE)
	{
		this(TYPE, DETERMINE_IMMUTABLE, new Object[0]);
	}
	
	public TypeToken2(final Type TYPE)
	{
		this(TYPE, false);
	}
	 ...
	 */
	public TypeToken2(final Object PARAMETERIZED_TYPE_INSTANCE)
	{
	...
	private TypeToken2<?> getArgument(final TypeVariable<?> TYPE_VARIABLE)
	{
	...
	public final TypeToken2<?> resolve(final TypeVariable<?> TYPE_VARIABLE, final Object... ARGUMENTS)
	{
	...
	private TypeToken2<?> resolve(final TypeVariable<?> TYPE_VARIABLE, final Type TYPE, final Object ARGUMENT)
	{
		if (ARGUMENT == null)
		{
		...
		else if (TYPE instanceof ParameterizedType)
		{
			...
			for (int index = 0; index < TYPE_ARGUMENTS.length; index++)
			{
				...
				{
					for (final Field FIELD : new TypeToken2<Object>(ARGUMENT.getClass( )).getInstanceFields( ))
					{
						try
						{
		...
		else if (TYPE instanceof TypeVariable)
		{
			...
			{
				return new TypeToken2<Object>(ARGUMENT.getClass( ), PARENT, DETERMINE_IMMUTABLE);
			}
			else
			{
				throw
	...
	Set<Field> getInstanceFields( )
	{
		...
		else
		{
			...
			( // Recursively get all inherited fields.
				new TypeToken2<Object>(RAW_TYPE.getSuperclass( )).getInstanceFields( )
			);
		...
		{ // Reflectively evaluate static immutability.
			if (isFinal(getRawType( ).getModifiers( )))
			{
				...
				for (final Field FIELD : getInstanceFields( ))
				{
					...
					{
						if (isImmutable(new TypeToken2<Object>(FIELD.getType( ))))
						{
							continue;
						}
						else
	...
	{
		final TypeToken2<?> CLASS = new TypeToken2<Object>(INSTANCE.getClass( ));
